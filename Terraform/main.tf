...
resource "azurerm_linux_web_app" "linux_webapp" {
  ...
  site_config {
    ...
    app_command_line = "apt-get update && apt-get install -y build-essential g++ && apt-get install libffi-dev && apt-get install cmake && pip install -r requirements.txt && gunicorn --bind 0.0.0.0:8000 --workers 3 application:app"
    application_stack {
      python_version = "3.9"
    }
  }
